  ...
  <div class="min-h-screen flex flex-col bg-background text-on-surface font-headline-lg">
    ...
    <main class="flex-grow pt-24 px-4 md:px-gutter max-w-container-max mx-auto w-full">
      ...
      <div class="mb-8">
        ...
        <h1 class="font-display-lg text-display-lg mt-4 text-on-surface">Championship Standings</h1>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-outline-variant bg-surface-container-low">
                <th class="px-6 py-4 font-label-sm text-label-sm uppercase tracking-widest text-on-surface-variant">Rank</th>
                <th class="px-6 py-4 font-label-sm text-label-sm uppercase tracking-widest text-on-surface-variant">Founder</th>
                <th class="px-6 py-4 font-label-sm text-label-sm uppercase tracking-widest text-on-surface-variant">Form</th>
                <th class="px-6 py-4 font-label-sm text-label-sm uppercase tracking-widest text-on-surface-variant">Maverick Picks</th>
                <th class="px-6 py-4 font-label-sm text-label-sm uppercase tracking-widest text-on-surface-variant text-right">Total Points</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-outline-variant">
              <tr
                v-for="(founder, index) in founderStats"
                :key="founder.id"
              ...
              >
                <td class="px-6 py-8">
                  <div class="flex items-center space-x-2">
                    <span class="font-bold text-headline-lg" :class="index === 0 ? 'text-primary' : 'text-on-surface-variant'">{{ index + 1 }}</span>
                    <span v-if="index === 0" class="material-symbols-outlined text-primary" style="font-variation-settings: 'FILL' 1;">crown</span>
                  </div>
                </td>
                <td class="px-6 py-8">
                  <div class="flex items-center space-x-4">
                    <div class="w-12 h-12 rounded-full overflow-hidden" :class="index === 0 ? 'border-2 border-primary' : 'border border-outline-variant'">
                    ...
                    <div>
                      ...
                      <div class="font-label-sm text-label-sm uppercase" :class="index === 0 ? 'text-primary' : 'text-on-surface-variant'">{{ founder.role }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-8">
                  <div class="flex space-x-1">
                    <div
                      v-for="(hit, formIndex) in founder.recentForm"
                      :key="`${founder.id}-form-${formIndex}`"
                ...
                <td class="px-6 py-8 text-right font-display-lg text-display-lg" :class="index === 0 ? 'text-primary gold-glow' : 'text-on-surface'">
                  {{ founder.totalPoints }}
                </td>
              </tr>
            </tbody>
      ...
      <section class="mb-20">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <!-- Maverick King Card -->
          <div class="lg:col-span-1 glass-card p-8 rounded-xl flex flex-col justify-between relative overflow-hidden group">
            ...
            <div>
              ...
              <h3 class="font-headline-lg text-headline-lg mb-2">Maverick King</h3>
            ...
            <div class="mt-8" v-if="maverickLeader">
              <div class="text-primary font-display-lg text-display-lg">{{ maverickLeader.name }}</div>
              <div class="flex items-center space-x-2 mt-2">
                <span class="px-3 py-1 bg-primary/20 text-primary font-label-sm text-label-sm rounded-full">{{ maverickLeader.maverickWins }} Maverick Wins</span>
          ...
          <div class="lg:col-span-2 glass-card p-8 rounded-xl">
            <div class="flex justify-between items-center mb-10">
              ...
              <span class="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-widest">Global Hit Rate</span>
            </div>
              ...
              <div v-for="founder in founderStats" :key="`${founder.id}-gauge`" class="flex flex-col items-center">
                <div class="relative w-32 h-32 mb-4">
                  <svg class="w-full h-full transform -rotate-90">
                    ...
                    <circle cx="64" cy="64" fill="transparent" r="58" stroke="#e1b638" stroke-dasharray="364.4" :stroke-dashoffset="gaugeOffset(founder.accuracy)" stroke-linecap="round" stroke-width="8"></circle>
                  </svg>
                  <div class="absolute inset-0 flex items-center justify-center font-display-lg text-headline-lg text-on-surface">{{ founder.accuracy }}%</div>
                </div>
                <div class="font-body-md text-body-md font-bold text-on-surface">{{ founder.name }}</div>
                <div class="text-label-sm font-label-sm text-on-surface-variant">{{ founder.correctPicks }} of {{ completedMatchesCount }} correct</div>
              </div>
            </div>
          </div>
        </div>
